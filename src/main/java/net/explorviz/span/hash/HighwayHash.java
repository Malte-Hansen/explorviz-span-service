...
/**
 * HighwayHash algorithm. See <a href="https://github.com/google/highwayhash"> HighwayHash on
 * GitHub</a>.
 * <p>
...
public final class HighwayHash {
   ...
   */
  public HighwayHash(final long key0, final long key1, final long key2, final long key3) {
   ...
   */
  public HighwayHash(final long[] key) {
    if (key.length != 4) {
  ...
  /**
   * Updates the hash with 32 bytes of data. If you can read 4 long values from your data
   * efficiently, prefer using update() instead for more speed.
   *
   ...
   */
  public void updatePacket(final byte[] packet, final int pos) {
    if (pos < 0) {
    ...
    final long a0 = read64(packet, pos);
    final long a1 = read64(packet, pos + 8);
    final long a2 = read64(packet, pos + 16);
    final long a3 = read64(packet, pos + 24);
    update(a0, a1, a2, a3);
  }

  /**
   * Updates the hash with 32 bytes of data given as 4 longs. This function is more efficient than
   * updatePacket when you can use it.
   *
  ...
  /**
   * Updates the hash with the last 1 to 31 bytes of the data. You must use updatePacket first per
   * 32 bytes of the data, if and only if 1 to 31 bytes of the data are not processed after that,
   * updateRemainder must be used for those final bytes.
   *
   ...
   */
  public void updateRemainder(final byte[] bytes, final int pos, final int size_mod32) {
    if (pos < 0) {
      throw new IllegalArgumentException(String.format("Pos (%s) must be positive", pos));
    }
    ...
    final int size_mod4 = size_mod32 & 3;
    final int remainder = size_mod32 & ~3;
    final byte[] packet = new byte[32];
    for (int i = 0; i < 4; ++i) {
      v0[i] += ((long) size_mod32 << 32) + size_mod32;
    }
    rotate32By(size_mod32, v1);
    System.arraycopy(bytes, pos, packet, 0, remainder);
    if ((size_mod32 & 16) != 0) {
      for (int i = 0; i < 4; i++) {
  ...
  private void reset(final long key0, final long key1, final long key2, final long key3) {
  ...
  private long zipperMerge0(final long v1, final long v0) {
    return (((v0 & 0xff000000L) | (v1 & 0xff00000000L)) >>> 24) | (
        ((v0 & 0xff0000000000L) | (v1 & 0xff000000000000L)) >>> 16) | (v0 & 0xff0000L) | (
        (v0 & 0xff00L) << 32) | ((v1 & 0xff00000000000000L) >>> 8) | (v0 << 56);
  }

  private long zipperMerge1(final long v1, final long v0) {
    return (((v1 & 0xff000000L) | (v0 & 0xff00000000L)) >>> 24) | (v1 & 0xff0000L) | (
        (v1 & 0xff0000000000L) >>> 16) | ((v1 & 0xff00L) << 24) | ((v0 & 0xff000000000000L) >>> 8)
        | ((v1 & 0xffL) << 48) | (v0 & 0xff00000000000000L);
  }

  private long read64(final byte[] src, final int pos) {
  ...
  private void rotate32By(final long count, final long[] lanes) {
    for (int i = 0; i < 4; ++i) {
      final long half0 = (lanes[i] & 0xffffffffL);
      final long half1 = (lanes[i] >>> 32) & 0xffffffffL;
  ...
  private void permuteAndUpdate() {
    update((v0[2] >>> 32) | (v0[2] << 32), (v0[3] >>> 32) | (v0[3] << 32),
        (v0[0] >>> 32) | (v0[0] << 32), (v0[1] >>> 32) | (v0[1] << 32));
  }
  ...
  private void processAll(final byte[] data, final int offset, final int length) {
    int i;
